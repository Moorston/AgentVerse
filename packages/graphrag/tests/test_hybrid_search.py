...
class TestHybridSearchFusionRanking:
    """Tests for fusion ranking formula."""
    ...
    async def test_vector_only_result_uses_vector_score(self, hybrid, mock_vector_search, mock_graph_search):
        ...
        mock_graph_search.search.return_value = []  # no graph matches

        results = await hybrid.search("query", top_k=10)

        assert len(results) == 1
        # No graph score, so combined = 0.6 * 0.8 + 0.4 * 0.0 = 0.48... wait
        # Actually looking at the code: combined_score = 0.6 * vector_score + 0.4 * graph_score
        # graph_score = graph_by_name.get(name, {}).get("score", 0.0) = 0.0
        # combined_score = 0.6 * 0.8 + 0.4 * 0.0 = 0.48
        # But source = "vector" because graph_score == 0
...
class TestHybridSearchDeduplication:
    ...
    async def test_deduplicates_by_name(self, hybrid, mock_vector_search, mock_graph_search):
        ...
        assert len(names) == 2
        assert names.count("Duplicate") == 1  # deduplicated
