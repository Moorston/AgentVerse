...
async def list_concepts(
    ...
    # Count total
    if category:
        count_result = await repo.execute_raw(
...
async def get_concept(
    ...
    repo = await ctx.get_repository()
    result = await repo.find_by_name("Concept", name)
    if not result:
...
async def get_neighbors(
    ...
    LIMIT 100
    """
    results = await repo.execute_raw(cql, {"name": name})
...
async def delete_concept(
    ...
    repo = await ctx.get_repository()
    await repo.delete_by_name("Concept", name)
